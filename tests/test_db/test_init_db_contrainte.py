...
class TestContraintesDB:
    ...
    def teardown_class(cls):
        """Fermeture de la connexion et du curseur à la fin de tous les tests"""
        cls.cur.close()
        cls.conn.close()

    def setup_method(self):
        """Sauvegarde de l'état de la base avant chaque test"""
    ...
    def test_fk_on_delete_cascade(self):
        ...
        self.cur.execute(
            f"""
            INSERT INTO {self.schema}.utilisateur (prenom, nom, mail, mdp)
            VALUES ('Clara','C','clara.cascade@mail.com', 'mdp')
        ...
        self.cur.execute(
            f"""
            DELETE FROM {self.schema}.utilisateur
        ...
        self.cur.execute(
            f"""
            SELECT 1 FROM {self.schema}.conversation
            WHERE id_proprio = %s;
            """,
            (id_user,),
        )
    ...
    def test_trigger_updates_timestamp_utilisateur(self):
        ...
        self.cur.execute(
            f"""
            INSERT INTO {self.schema}.utilisateur (prenom, nom, mail, mdp)
            VALUES ('Clara', 'C', 'clara.trigger@mail.com', 'mdp')
        ...
        # Mise à jour du mail (valeur unique)
        ...
        ), "Le trigger 'set_updated_at' n'a pas été exécuté sur utilisateur."
    ...
    def test_trigger_updates_timestamp_conversation(self):
        ...
        assert (
            deuxieme_temps > premier_temps
        ), "Le trigger 'set_updated_at' n'a pas été exécuté sur conversation."
    ...
    def test_default_timestamps(self):
        ...
        sont bien initialisés automatiquement à l'insertion.
        """
        self.cur.execute(
            f"""
            INSERT INTO {self.schema}.utilisateur (prenom, nom, mail, mdp)
